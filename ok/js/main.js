...
function buildAssistConcessionItems(from, to, openCostTotal, decimalPlaces, fixedTierOpenCost = null, stopPrice = null) {
  ...
  const overrideStop = toNumber(stopPrice);
  const stop = overrideStop != null ? overrideStop : from;
...
const METHODOLOGY_SECTIONS = [
  ...
  },
  {
    title: '2、核心理念',
    items: [
      '右侧交易，趋势立项。风控第一，收益第二，策略唯一。',
      '确定性第一，风险第二，盈亏比第三。做胜率最高的选择，不做盈利最大的选择。',
      '挂单交易。能成交就做，不能成交就算了，机会不止一个。',
      '没有体系结论就不操作。没有明确信号时，不操作就是最好的操作。',
      '行情很简单，复杂的是人心。在限定的范围内做正确的事。',
      '方法已经验证过，循环执行，不再改规则。等待就是最快的前行。',
    ],
  },
  {
    title: '3、选择标准',
    items: [
      ...
      '大维度是主方向，小维度只做配合。方向统一看10，背离也看10。',
      '只研究热度前20、交易量过亿的标的。开单只做前三，种类不超过三个。',
      '只做龙头的日内右侧趋势。放弃多空临界值，不做BTC和ETH。',
      '只看自己的行情和数据，不参考别人。',
      '止损位置，往前数10个标的里的插针极限。',
      '优先级：趋势立项＞趋势力预测。反趋势只做空，而且时间和空间都要满足；新的趋势立项优先于反趋势。',
    ],
  },
  {
    title: '4、时间',
    items: [
      '4小时为主，日线为辅。顺序看4h，倒序看1d。',
      '15分钟和1小时不能作为操作依据。',
      '操作节点是8点、16点、24点，可以提前半小时准备。节点之间不看、不操作。',
      '盯盘会调动情绪。按挂单列表和时间执行，不用盯盘代替判断。',
      '行情剧烈时执行，行情平缓时思考。关键点出现之前只是猜测，出现之后才计算。',
    ],
  },
  {
    title: '5、仓位',
    items: [
      '计划是3个目标 × 3个档位。子弹集中在当时最优势的单子上，不摊薄，也不纠结利用率。',
      '三档挂单。只挂前一半优势单，劣势单最多用1个。',
      '单笔仓位不超过本金的10%。再大，心态会失衡，时间和空间就不起作用。',
      '单笔止损幅度不超过50%，止盈预期不超过一个数量级。',
      '开仓之后仓位固定：不补、不减、不滚。时间才是杠杆。',
    ],
  },
  {
    title: '6、持仓与出场',
    items: [
      '空仓才分析，持仓后不再分析，只信最初的判断。',
      '浮亏扛到底。浮亏会扭曲判断，所以不用它做新决定；趋势还在，这个位置就是机会。踏空才是要避开的风险。',
      '利润可以随时出，不必吃满，也不必开在最好的位置。',
      '时间上看九尾和十尾，空间上看3倍和5倍。这是参考，不是必须拿到的目标。',
      '短线1个单位解决战斗。',
      '趋势立项不改最初判断。反趋势见好就收。',
      '正确但不准确是常态。止损用来挡住小概率意外，不用来回做波段。',
    ],
  },
  {
    title: '7、模型',
    items: [
      '生命周期：趋势立项，到趋势力回调，再到调整等待，然后进入下一个周期。对不上这个过程，就不做。',
      '趋势立项前期挂单。拐点10用来避险和验证，验证之后再看回调。',
      '四种形态都用这一套看：趋势立项、反趋势、追涨杀跌、极限逼空。',
      '时间和空间一起走到10，才把行情看成单边。',
      '分析时对照大小周期、形态和数值、理论和实际、过程和结果。执行时不做波段。',
      '数据是用来帮助判断的，不是用来把自己困住。历史数据不要轻易删。',
      '咒语，验证，结局。',
    ],
  },
  {
    title: '8、基本定律',
    items: [
      '盈亏比和确定性互相排斥。确定性已经很高时，剩下的盈亏比往往不大。',
      '做多赢得多，做空盈利快。盈亏同源。日内30%基本是极限。',
      '以大博小。确定性永远排在第一位，以小博大经不起连续失误。',
      '顺境时不要张狂，逆境时不要焦虑。风险和收益一起看，赢的时候也留余地。',
      '见路不走，宽门不进，肥田不耕。',
      '一开始就要接受可能出现的全部结果。错过一些机会，说明风控在起作用。',
    ],
  },
  {
    title: '9、心态',
    items: [
      '长期计划不变：35岁之前退休。手头只盯当前这个小目标。',
      '没到100万都是欢乐豆。本金会剧烈波动，这是把风险限定之后的正常现象。',
      '走渐进，不走螺旋。先保证生存，再谈盈利。',
      '不后悔，不遗憾。钝感力，松弛感。心态不好就停下来，去场外。',
      '最危险的不是市场，是自己的心。真正的考验是关键位置还按不按规则做。',
...
function collectAdminNameCounts(rows) {
  const list = filterRowsByAdminTimeFilter(getAdminVisibleRows(rows));
